...
CREATE TABLE IF NOT EXISTS spinupfy_sites (
  ...
  -- Pricing & payment
  duration_days     INTEGER NOT NULL DEFAULT 1,   -- calculated on insert/update
...
-- Indexes for lifecycle cron queries
...
COMMENT ON COLUMN spinupfy_sites.duration_days IS 'Auto-computed from end_date - start_date on insert/update';
COMMENT ON COLUMN spinupfy_sites.reminder_3day_sent IS 'True after 3-day expiry reminder email has been sent';
COMMENT ON COLUMN spinupfy_sites.reminder_1day_sent IS 'True after 1-day expiry reminder email has been sent';
COMMENT ON COLUMN spinupfy_sites.suspended_at IS 'Set when site is suspended after end_date passes';
COMMENT ON COLUMN spinupfy_sites.deleted_at IS 'Set when site is permanently deleted after 5-day grace period';
